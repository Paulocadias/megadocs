...
document.addEventListener('DOMContentLoaded', () => {
    ...
    async function processFile(file, options) {
        return new Promise((resolve, reject) => {
            ...
            formData.append('inject_memory', 'true');  // Always inject to memory for RAG
            ...
            xhr.withCredentials = true;  // Ensure session cookies are sent
    ...
    async function updateMemoryStatus(count) {
        ...
        if (memoryStatus && memoryCount) {
            if (count > 0) {
                ...
                try {
                    const response = await fetch('/api/memory/status', { credentials: 'include' });
                    const data = await response.json();
