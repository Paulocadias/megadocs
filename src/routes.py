...
from security import (
    ...
    release_conversion_slot, get_active_conversions, validate_file_size
)
from utils import (
...
from openrouter_gateway import chat_completion, chat_completion_with_fallback, analyze_image, image_to_text_description
from memory_store import get_memory_store
from assistant import ask_assistant
from middleware.xray import xray_trace, update_xray_from_gateway, update_xray_from_agent
from router import get_router
from sql_sandbox import SQLSandboxSession
from agents.investigator import InvestigatorAgent
from evaluator import ResponseEvaluator, passes_quality_gate
import threading
...
def convert_file():
    ...
    # File size validation (pre-read check)
    size_error = validate_file_size(content_length=request.content_length, ip=ip, request_id=request_id)
    if size_error:
        return jsonify(size_error[0]), size_error[1]

    # Read file content for magic byte validation
    file_content = file.read()
    file.seek(0)  # Reset for saving

    # File size validation (post-read check)
    size_error = validate_file_size(actual_size=len(file_content), ip=ip, request_id=request_id)
    if size_error:
        return jsonify(size_error[0]), size_error[1]
...
@bp.route("/api/memory/debug", methods=["GET"])
def memory_debug():
    """Debug endpoint: show full RAG context that would be sent to LLM."""
    memory_store = get_memory_store()
    session_id = session.get('user_id', '')
    ...
    return jsonify({
        ...
        "combined_context_length": len(context),
        "combined_context_preview": context[:1000] + "..." if len(context) > 1000 else context
    })
...
def api_chat():
    ...
    try:
        ...
        data = request.json
        user_model = data.get('model')  # User's explicit model choice (may be None)
        auto_route = data.get('auto_route', False)  # Smart routing flag
        messages = data.get('messages', [])
        context = data.get('context', '')
        routing_decision = None  # Will be set if auto_route is enabled
        ...
        # Debug log: verify context is being loaded
        context_preview = context[:500] if context else "EMPTY"
        logger.info(f"RAG Context loaded: {len(context)} chars, preview: {context_preview}...", extra={'request_id': request_id})
        ...
        try:
            ...
            # Smart Router: Automatically select model based on prompt complexity
            if auto_route and not user_model:
                router = get_router()
                ...
                logger.info(
                    f"Smart Router: {routing_decision.complexity.value} -> {model} "
                    f"(confidence: {routing_decision.confidence:.0%})",
                    extra={'request_id': request_id}
                )
            else:
                model = user_model or 'Google Gemini 2.0 Flash'

            result = chat_completion_with_fallback(model=model, messages=messages, context=context, domain=domain)

            # Update X-Ray trace with gateway metrics
            ...
            # v3.0: Record LLM request with cost tracking (CFO Hook)
            ...
            stats.record_llm_request(
                ...
                gpt4_cost=result.get("gpt4_cost", 0.0),
            ...
            # v3.2: Optional response evaluation (Evaluation-Driven Development)
            evaluation_result = None
            should_evaluate = request.args.get('evaluate') == '1' or request.headers.get('X-Evaluate') == '1'
            if should_evaluate and result.get("response"):
                try:
                    ...
                    stats.record_evaluation(
                        ...
                        dimensions={name: {"score": dim.score, "reasoning": dim.reasoning}
                                    for name, dim in eval_result.dimensions.items()},
                        feedback=eval_result.feedback,
                        eval_latency_ms=eval_result.latency_ms,
                        eval_model=eval_result.model_used,
                        eval_tokens=eval_result.tokens_used,
                        eval_cost=eval_result.cost,
                        request_id=request_id,
                        threshold=7.0,
                        ip=ip
                    )
                    logger.info(f"Evaluation completed: score={eval_result.overall_score:.1f}",
                               extra={'request_id': request_id})
                except Exception as eval_error:
                    logger.warning(f"Evaluation failed: {eval_error}", extra={'request_id': request_id})
            ...
            # Include routing decision if smart routing was used
            if routing_decision:
                response_data["routing"] = routing_decision.to_dict()

            # Include evaluation result if requested
            if evaluation_result:
                response_data["_evaluation"] = evaluation_result
...
# =============================================================================
# SQL SANDBOX (BYOD - Bring Your Own Database)
# =============================================================================

# Session storage for SQL sandbox instances
_sql_sessions: dict[str, SQLSandboxSession] = {}


def _get_sql_session(session_id: str) -> SQLSandboxSession:
    """Get or create SQL sandbox session for user."""
    if session_id not in _sql_sessions:
        _sql_sessions[session_id] = SQLSandboxSession(session_id)
    return _sql_sessions[session_id]


@bp.route("/sql-sandbox")
def sql_sandbox_page():
    """Render SQL Sandbox interface."""
    if 'csrf_token' not in session:
        session['csrf_token'] = hashlib.sha256(os.urandom(32)).hexdigest()

    return render_template("sql_sandbox.html", csrf_token=session['csrf_token'], active_page='sql-sandbox')


@bp.route("/api/sql/consent", methods=["POST"])
@security_check
def sql_consent():
    """Record user consent for SQL sandbox data processing."""
    request_id = getattr(request, 'request_id', 'UNKNOWN')
    ...
    try:
        ...
        if not consent:
            return jsonify({
                "error": "Consent is required to use SQL Sandbox",
                "request_id": request_id
            }), 400

        # Store consent in session
        session['sql_consent'] = True
        session['sql_consent_time'] = __import__('datetime').datetime.now().isoformat()

        # Also initialize SQL session with consent
        user_id = session.get('user_id')
        if not user_id:
            user_id = hashlib.sha256(os.urandom(32)).hexdigest()[:16]
            session['user_id'] = user_id
        sql_session = _get_sql_session(user_id)
        sql_session.give_consent()

        logger.info(f"SQL Sandbox consent granted", extra={'request_id': request_id})
        return jsonify({
            "success": True,
            "message": "Consent recorded. You may now upload database files.",
            "request_id": request_id
        })

    except Exception as e:
        logger.error(f"SQL consent error: {e}", extra={'request_id': request_id})
        return jsonify({"error": "Failed to record consent", "request_id": request_id}), 500


@bp.route("/api/sql/upload", methods=["POST"])
@security_check
def sql_upload():
    """Upload and ingest database file for SQL sandbox."""
    request_id = getattr(request, 'request_id', 'UNKNOWN')
    ip = get_client_ip()

    # Check consent - with fallback for session persistence issues
    # The frontend shows a consent dialog, so if they're uploading, they've consented
    if not session.get('sql_consent'):
        # Auto-grant consent if user is uploading (they clicked through the UI)
        session['sql_consent'] = True
        session['sql_consent_time'] = __import__('datetime').datetime.now().isoformat()
        logger.info(f"SQL Sandbox consent auto-granted on upload", extra={'request_id': request_id})
    ...
    # Extension validation
    ext = Path(filename).suffix.lower()
    allowed_sql_extensions = {'.db', '.sqlite', '.sqlite3', '.sql', '.csv', '.xlsx'}
    if ext not in allowed_sql_extensions:
        return jsonify({
            "error": f"File type '{ext}' not supported. Allowed: {', '.join(sorted(allowed_sql_extensions))}",
            "request_id": request_id
        }), 400

    # Size check (use config limit for SQL sandbox)
    max_size = Config.MAX_CONTENT_LENGTH
    max_mb = max_size // (1024 * 1024)
    file_content = file.read()
    if len(file_content) > max_size:
        return jsonify({
            "error": f"File too large. Maximum size is {max_mb}MB. Your file is {len(file_content) / (1024*1024):.1f}MB.",
            "request_id": request_id
        }), 413
    ...
    try:
        ...
        # Ensure consent is synced (handles server restart case)
        if not sql_session.has_consent:
            sql_session.give_consent()

        # Ingest file
        result = sql_session.ingest(file_content, filename)
        ...
        # Record stats
        stats.record_sql_upload(ip, ext, len(file_content), success=True)

        logger.info(f"SQL file ingested: {filename} from {ip}", extra={'request_id': request_id})
        return jsonify({
            "success": True,
            "filename": filename,
        ...
        stats.record_sql_upload(ip, ext if 'ext' in dir() else 'unknown', 0, success=False)
        logger.error(f"SQL upload error: {e}", extra={'request_id': request_id})
        return jsonify({"error": f"Failed to process file: {str(e)}", "request_id": request_id}), 500


@bp.route("/api/sql/query", methods=["POST"])
@security_check
def sql_query():
    """Execute natural language query on uploaded database."""
    request_id = getattr(request, 'request_id', 'UNKNOWN')
    ip = get_client_ip()

    # Check consent - with fallback for session persistence issues
    if not session.get('sql_consent'):
        # Auto-grant consent if user is querying (they've already uploaded via UI)
        session['sql_consent'] = True
        session['sql_consent_time'] = __import__('datetime').datetime.now().isoformat()
        logger.info(f"SQL Sandbox consent auto-granted on query", extra={'request_id': request_id})
    ...
    try:
        ...
        if len(question) > 1000:
            return jsonify({"error": "Question too long (max 1000 characters)", "request_id": request_id}), 400
        ...
        if not sql_session.has_database():
            return jsonify({
                "error": "No database loaded. Please upload a file first.",
                "request_id": request_id
            }), 400
        ...
        # Execute query
        result = sql_session.query(question)
        ...
        stats.record_sql_query(ip, latency_ms, success=True)

        # Build response with fallback info if applicable
        response_data = {
            "success": True,
            "sql": result.get("sql", ""),
        ...
        # Add fallback info if a fallback model was used
        if result.get("fallback_used"):
            response_data["fallback_used"] = True
            logger.info(f"SQL query used fallback model: {result.get('model')}", extra={'request_id': request_id})

        logger.info(f"SQL query executed from {ip}: {question[:50]}...", extra={'request_id': request_id})
        return jsonify(response_data)

    except Exception as e:
        stats.record_sql_query(ip, 0, success=False)
        logger.error(f"SQL query error: {e}", extra={'request_id': request_id})
        return jsonify({"error": f"Query failed: {str(e)}", "request_id": request_id}), 500


@bp.route("/api/sql/schema", methods=["GET"])
def sql_schema():
    ...
    try:
        user_id = session.get('user_id')
        if not user_id:
            return jsonify({"tables": [], "request_id": request_id})
        ...
        logger.error(f"SQL schema error: {e}", extra={'request_id': request_id})
        return jsonify({"error": "Failed to get schema", "request_id": request_id}), 500


@bp.route("/api/sql/clear", methods=["POST"])
@security_check
def sql_clear():
    """Clear SQL sandbox session data."""
    request_id = getattr(request, 'request_id', 'UNKNOWN')

    try:
        user_id = session.get('user_id')
        if user_id and user_id in _sql_sessions:
            _sql_sessions[user_id].cleanup()
            del _sql_sessions[user_id]

        # Clear consent
        session.pop('sql_consent', None)
        session.pop('sql_consent_time', None)

        logger.info(f"SQL session cleared", extra={'request_id': request_id})
        return jsonify({
            "success": True,
        ...
        logger.error(f"SQL clear error: {e}", extra={'request_id': request_id})
        return jsonify({"error": "Failed to clear session", "request_id": request_id}), 500


@bp.route("/api/sql/stats", methods=["GET"])
def sql_stats():
    """Get SQL Sandbox usage statistics."""
    request_id = getattr(request, 'request_id', 'UNKNOWN')

    try:
        sql_stats = stats.get_sql_sandbox_stats()
        return jsonify({
            "success": True,
            "stats": sql_stats,
            "request_id": request_id
        ...
        logger.error(f"SQL stats error: {e}", extra={'request_id': request_id})
        return jsonify({"error": "Failed to get stats", "request_id": request_id}), 500
...
def api_investigate():
    """
    Execute investigation agent on uploaded documents.

    Accepts:
    - mission: Investigation mission (e.g., "Identify all legal risks")
    - model: Optional model override

    Returns:
    - success: bool
    - report: Structured findings with sources
    - steps_completed: Number of agent steps completed (4 = full workflow)
    - total_latency_ms: Total execution time
    - cost: Actual cost
    - savings_percent: Savings vs GPT-4
    - _debug: Agent telemetry (when ?debug=1)
    """
    request_id = getattr(request, 'request_id', 'UNKNOWN')
    ip = get_client_ip()
    ...
    try:
        ...
        if not mission:
            return jsonify({
                "error": "Mission is required. Example: 'Identify all legal risks in this contract'",
                "request_id": request_id
            }), 400

        if len(mission) > 500:
            return jsonify({
                "error": "Mission too long (max 500 characters)",
                "request_id": request_id
            }), 400

        # Get user session for document retrieval
        user_id = session.get('user_id')
        if not user_id:
            user_id = hashlib.sha256(os.urandom(32)).hexdigest()[:16]
            session['user_id'] = user_id

        logger.info(f"Investigation started: {mission[:50]}...", extra={
            'request_id': request_id,
            'ip': ip,
            'session_id': user_id[:8]
        })

        # Create and run agent
        agent = InvestigatorAgent(session_id=user_id, model=model)
        result = agent.run(mission)

        # Update X-Ray telemetry with agent data
        ...
        stats.record_agent_execution(
            agent_type="investigator",
            mission=mission,
            steps_completed=result.get('steps_completed', 0),
            total_latency_ms=result.get('total_latency_ms', 0),
            actual_cost=result.get('cost', 0.0),
            gpt4_cost=report.get('gpt4_cost', 0.0),
            savings=report.get('savings', 0.0),
            savings_percent=result.get('savings_percent', 0.0),
            documents_count=len(debug.get('documents_analyzed', [])),
            chunks_retrieved=debug.get('chunks_retrieved', 0),
            findings_count=len(report.get('findings', [])),
            quality_score=report.get('quality', {}).get('quality_score'),
            success=result.get('success', False),
            ip=ip
        )

        # Include debug info if requested
        include_debug = request.args.get('debug') == '1' or request.headers.get('X-Debug') == '1'
        if not include_debug and '_debug' in result:
            del result['_debug']

        result['request_id'] = request_id

        logger.info(f"Investigation completed: {result.get('steps_completed', 0)} steps, {result.get('total_latency_ms', 0)}ms",
                   extra={'request_id': request_id})

        return jsonify(result)

    except Exception as e:
        logger.exception(f"Investigation endpoint error: {e}", extra={'request_id': request_id})
        return jsonify({
            "success": False,
