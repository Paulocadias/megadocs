...
def validate_file_size(content_length: int = None, actual_size: int = None, ip: str = "unknown", request_id: str = "UNKNOWN"):
    """
    Validate file size against max limit.

    Args:
        content_length: Content-Length header value (optional, for pre-read check)
        actual_size: Actual file size after reading (optional, for post-read check)
        ip: Client IP for logging
        request_id: Request ID for logging

    Returns:
        None if OK, or tuple (error_dict, status_code) if rejected
    """
    max_size = Config.MAX_CONTENT_LENGTH
    max_mb = max_size // (1024 * 1024)

    # Check Content-Length header first (pre-read)
    ...
    # Check actual size after reading (post-read)
    if actual_size is not None and actual_size > max_size:
...
def validate_file_magic(file_content, extension):
    ...
    text_extensions = {'.txt', '.csv', '.json', '.xml', '.sql'}
