...
@pytest.mark.unit
@pytest.mark.skip(reason="Chunker tests timeout in pytest on Windows - tiktoken conflict")
class TestChunkDocument:
    ...
    def test_multiple_chunks_for_long_text(self):
        ...
        from chunker import chunk_document
        # Create text that will produce multiple chunks (short for test performance)
        text = "A" * 600  # Will be split with char_chunk_size=400 (chunk_size*4)
    ...
    def test_overlap_validation(self):
        ...
        from chunker import chunk_document
        text = "A" * 600  # Minimal text that produces multiple chunks
        result = chunk_document(
            text,
            chunk_size=100,
...
class TestChunkStrategies:
    ...
    def test_character_strategy(self):
        ...
        from chunker import chunk_document
        text = "A" * 200  # Short text for performance
        result = chunk_document(text, strategy="character")
        assert "error" not in result
...
class TestChunkStructure:
    ...
    def test_chunk_indices_are_sequential(self):
        ...
        from chunker import chunk_document
        text = "A" * 600  # Short text that produces multiple chunks
...
class TestTokenCounting:
    ...
    def test_token_estimation_fallback(self):
        ...
        from chunker import get_token_count
        result = get_token_count("Test text for estimation")
        assert result["token_count"] > 0
